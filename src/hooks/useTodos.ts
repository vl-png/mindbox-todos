...
export function useTodos(initialStateOverride: State = initialState) {
  const [state, dispatch] = useReducer(reducer, initialStateOverride, (seedState) => {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      return raw ? { ...seedState, todos: JSON.parse(raw) as Todo[] } : seedState;
    } catch {
      return seedState;
    }
  });
  ...
  const remaining = useMemo(() => state.todos.filter(todo => !todo.completed).length, [state.todos]);
  ...
  return {
    state, visible, remaining, completed,
    add: (title: string) => {
      const trimmedTitle = title.trim();
      if (!trimmedTitle) return;
      const todo: Todo = { id: crypto.randomUUID(), title: trimmedTitle, completed: false };
      dispatch({ type: 'add', todo });
    },
    toggle: (id: string) => dispatch({ type: 'toggle', id }),
    remove: (id: string) => dispatch({ type: 'remove', id }),
    edit: (id: string, title: string) => dispatch({ type: 'edit', id, title }),
    toggleAll: (completed: boolean) => dispatch({ type: 'toggleAll', completed }),
